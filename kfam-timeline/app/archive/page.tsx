            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {dates.map(date => (
                <Link key={date} href={`/archive/${date.replaceAll("/", "-")}`} className="group p-8 bg-card rounded-[2rem] border border-card-border hover:border-accent transition-all hover:shadow-xl flex justify-between items-center">
                  ...
                  <div className="w-6 h-6 rounded-full bg-background flex items-center justify-center group-hover:bg-accent-soft group-hover:text-accent text-mute transition-all text-xl"><ArrowRight className="w-4 h-4" /></div>
                </Link>
              ))}
            </div>
          </div>
